...
public class Board {
  ...
  Array<EffectSlide> statusPlayer;
    ...
    newGameBtn.addListener(new ClickListener(){
      ...
      public void clicked(InputEvent event, float x, float y) {
        ...
        for(int i = 0; i < game.bots.size; i++) {
          game.bots.get(i).disposeParticle();
        }
        newGameBtn.setVisible(false);
        game.checkVideo(2);
        replay();
...
//                                statusPlayer.get(0).setVisible(true);
  ...
  private void checkPointCardAtTheStart(){
    ...
    if(resutl.x == 0 && (resutl.y == 1 || resutl.y == 2)){
      ...
      if(GGameStart.mode == 1){
        checkPointCards1();
        checkMoneyBotPlayer();
        int rs = checkCardsWidthPlayer(cards.get(0), idBoss);
        isPlayerWin = rs == -1 ? true : false;
  ...
  private void initCards(){
    ...
    if(turnInitCards == GGameStart.member*2 && cards.get(turnInitCards%GGameStart.member).size == 2 ){
      ...
      Card card;
//            if(turnInitCards%GGameStart.member == 0 && dem == 0){
//                card = new Card(gameMainAtlas, cg.get(turnInitCards%GGameStart.member),3, 1);
//                dem++;
//            } else if (turnInitCards%GGameStart.member == 0 && dem == 1) {
//                card = new Card(gameMainAtlas, cg.get(turnInitCards%GGameStart.member),2, 9);
//            }
//           else
      ...
      if(turnBossCheck != 0){
        ...
        game.groupPocker.get(turnBossCheck - 1).addAction(sequence(
                ...
                GSimpleAction.simpleAction((d, a)->{
                  ...
                  game.bots.get(turnBossCheck - 1).lose();
      ...
      if(turnBossCheck != 0){
        ...
        game.groupPockerTemp.get(turnBossCheck - 1).addAction(sequence(
                ...
                GSimpleAction.simpleAction((d, a)->{
                  ...
                  game.bots.get(turnBossCheck - 1).win();
...
//                            statusPlayer.get(0).setVisible(true);
      ...
      if(turnBossCheck != 0){
        ...
        game.bots.get(turnBossCheck - 1).tie();
        turnBossCheck++;
        bossBotAction();
      }
      else {
//                statusPlayer.get(2).setVisible(true);
  ...
  private void dispose(){
    ...
    if(GGameStart.mode == 0){
      ...
      imageBoss.remove();
      for(EffectSlide effectSlide : statusPlayer){
        effectSlide.remove();
      }
    }
  ...
  private void winParticle(){
    EffectSlide win = new EffectSlide("win", GMain.screenWidth/2 - 250, GMain.screenHeight - 100, group);
    statusPlayer.add(win);
    win.start();
    group.addActor(win);
  }

  private void loseParticle(){
    EffectSlide lose = new EffectSlide("lose", GMain.screenWidth/2 - 250, GMain.screenHeight - 100, group);
    statusPlayer.add(lose);
    lose.start();
    group.addActor(lose);
  }

  private void tieParticle(){
    EffectSlide tie = new EffectSlide("tie", GMain.screenWidth/2 - 250, GMain.screenHeight - 100, group);
    statusPlayer.add(tie);
    tie.start();
    group.addActor(tie);
  }
...
//  private void showPanelAds(boolean isWin){
//    if(isWin){
//      if(GMain.platform.isVideoRewardReady()){
//      Image panel, btnDone, btnExit;
//      Group groupads = new Group();
//      GStage.addToLayer(GLayer.top, groupads);
//
...
//      groupads.addActor(blackOverlay);
//      blackOverlay.setColor(0, 0, 0, 0.7f);
//
//
//        panel = GUI.createImage(atlasAds, "seeAdsAlert");
//        btnDone = GUI.createImage(atlasAds, "btnDone");
//        btnExit = GUI.createImage(atlasAds, "btnExit");
//        groupads.addActor(panel);
//        groupads.addActor(btnDone);
//        groupads.addActor(btnExit);
//        //panel.setOrigin(Align.center);
//        panel.setPosition(GMain.screenWidth/2, GMain.screenHeight/2, Align.center);
//        btnDone.setPosition(GMain.screenWidth/2 + btnDone.getWidth()/2, GMain.screenHeight/2 + GMain.screenHeight/10, Align.center);
//        btnExit.setPosition(GMain.screenWidth/2 - btnExit.getWidth()/2, GMain.screenHeight/2 + GMain.screenHeight/10, Align.center);
//
...
//            btnExit.addAction(Actions.sequence(
//              Actions.scaleTo(0.3f, 0.3f, 0.05f, swingIn),
//              Actions.scaleTo(1, 1, 0.05f, swingOut),
//              GSimpleAction.simpleAction((d, a)->{
//                groupads.remove();
//                groupads.clear();
//                return true;
...
//    //atlasAds = GAssetsManager.getTextureAtlas("adsPanel.atlas");
//  }
}
